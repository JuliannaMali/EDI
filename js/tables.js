...
//fetch('Swieczki (2).json')
  ...
  .then(data => {
    outsideCandles = data;
    data.forEach(candle => {
      const try1 = `<tr><div id="tableContent" class="collapse"><td>${candle.model}</td><td>${candle.wielkosc}</td><td>${candle.zapach}</td><td>${candle.wosk}</td><td>${candle.dostepnosc}</td><td>${candle.kod}</td><td>${candle.iloscsprzedanych}</td><td>${candle.dataostatniejsprzedazy}</td></div></tr>`;
